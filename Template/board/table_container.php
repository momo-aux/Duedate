    ...
    <?php else: ?>
        ...
        <?php else: ?>
            <table id="board"
                   ...
                   data-save-url="<?= $this->url->href('DueDateAjaxController', 'save', array('project_id' => $project['id'],'plugin' => 'Duedate')) ?>"
                   data-reload-url="<?= $this->url->href('DueDateAjaxController', 'reload', array('project_id' => $project['id'],'plugin' => 'Duedate')) ?>"
                   data-check-url="<?= $this->url->href('DueDateAjaxController', 'check', array('project_id' => $project['id'], 'timestamp' => time(),'plugin' => 'Duedate')) ?>"
        ...
        <?php foreach ($swimlanes as $index => $swimlane): ?>
            <?php if (! ($swimlane['nb_tasks'] === 0 && isset($not_editable))): ?>
                ...
                <?php if ($index > 0 && $swimlane['nb_swimlanes'] > 1): ?>
                    <?= $this->render('Duedate:board/table_swimlane', array(
                        'project' => $project,
                        'swimlane' => $swimlane,
                ...
                <?= $this->render('Duedate:board/table_column', array(
                    'swimlane' => $swimlane,
                    'not_editable' => isset($not_editable),
                )) ?>
                ...
                <?= $this->render('Duedate:board/table_tasks', array(
                    'project' => $project,
                    'swimlane' => $swimlane,
                    'not_editable' => isset($not_editable),
